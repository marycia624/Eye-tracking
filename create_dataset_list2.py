...
def create_dataset_lists():
    ...
    dataset_unzip = 'gazecapture_unzip'
    output_root = 'dataset'
    ...
    for dir in dirs[:]:
        ...
        file.extractall(join(dataset_unzip))
        file.close()
        # open json files
        face_file = open(join(dataset_unzip, newdir, "appleFace.json"))
        left_file = open(join(dataset_unzip, newdir, "appleLeftEye.json"))
        right_file = open(join(dataset_unzip, newdir, "appleRightEye.json"))
        frames_file = open(join(dataset_unzip, newdir, "frames.json"))
        info_file = open(join(dataset_unzip, newdir, "info.json"))
        ...
        if info_json["Dataset"] == "train":
            output = open(join(output_root, "train", os.path.basename(newdir)), "w+")

        if info_json["Dataset"] == "test":
            output = open(join(output_root, "test", os.path.basename(newdir)), "w+")

        if info_json["Dataset"] == "val":
            output = open(join(output_root, "validation", os.path.basename(newdir)), "w+")
        ...
        for i in range(0, int(info_json["TotalFrames"])):
            if left_json["IsValid"][i] and right_json["IsValid"][i] and face_json["IsValid"][i]:
                output.write(os.path.basename(newdir) + "/" + frames_json[i])
                output.write("\n")
